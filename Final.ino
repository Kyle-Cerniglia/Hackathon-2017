...
bool canForward(){
  //If we are north,
  if(curdir%4 == 1 && grid[xpos][ypos+1] && ypos+1 < 32){
  return true;
  }
  //If we are east,
  if(curdir%4 == 2 && grid[xpos+1][ypos] && xpos+1 < 32){
      return true;
  }
  //If we are south,
  if(curdir%4 == 3 && grid[xpos][ypos-1] && ypos - 1 > 0){
      return true;
  }
  //If we are west,
  if(curdir%4 == 0 && grid[xpos-1][ypos] && xpos -1 > 0){
      return true;
  }
  return false;
}
